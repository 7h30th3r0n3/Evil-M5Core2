...
int maxChannel = 14;  // Canaux Wi-Fi max pour 2,4 GHz
...
void setup() {
  ...
  WiFi.mode(WIFI_STA);
  Serial.println("ESP32-C3 Hopping Channels & Deauth Packets");
}
  ...
  if ((millis() - lastTime) > timerDelay) {
    ...
    if (n == 0) {
      ...
      for (int8_t i = 0; i < n; i++) {
        ...
        delay(200);  // Attendre 100 ms pour que la LED reste allumée brièvement
